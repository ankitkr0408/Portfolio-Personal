...
const DEFAULT_WORDS = ["HELLO", "I'M ANKIT", "WELCOME", "ENJOY"];
...
export function ParticleTextEffect({
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const fontSize = Math.max(48, Math.min(canvas.width / 10, canvas.height / 5));
    ...
    const isFirstCall = particles.length === 0;
    let particleIndex = 0;
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          if (isFirstCall) {
            // Spawn inside the viewport so the first word is visible immediately
            // instead of flying in from far off-screen against a black background.
          ...
          particle.colorBlendRate = Math.random() * 0.04 + 0.025;

          if (isFirstCall) {
            // Skip the black-to-color fade-in on first word; colorWeight=1 means
            // particles render at targetColor immediately.
  ...
  const animate = () => {
    ...
    // Wall-clock word advancement: stays in sync with the parent overlay's
    // setTimeout-based fade even when frames drop (Spline + particles share GPU).
    const elapsed = performance.now() - startTimeRef.current;
    const targetIndex = Math.floor(elapsed / wordDurationMs) % words.length;
    if (targetIndex !== wordIndexRef.current) {
      wordIndexRef.current = targetIndex;
      nextWord(words[targetIndex], canvas);
    }
